...
class RoomGrid(MiniGridEnv):
    ...
    def add_object(self, i, j, kind=None, color=None, **kwargs):
        """
        ...
        assert kind in ["key", "ball", "box", "goal", "reward"]
        if kind == "key":
            obj = Key(color, **kwargs)
        elif kind == "ball":
            obj = Ball(color, **kwargs)
        elif kind == "box":
            obj = Box(color, **kwargs)
        elif kind == "goal":
            obj = Goal(**kwargs)
        elif kind == "reward":
            obj = Reward(color, **kwargs)
    ...
    def connect_all(
        self,
        door_colors=COLOR_NAMES,
        max_itrs=5000,
        within_timesteps=None,
        self_locking=False,
    ):
        """
        ...
        def find_reach():
            ...
            while len(stack) > 0 and (
                within_timesteps is None or t <= within_timesteps
            ):
                t += 1
                room = stack.pop()
                if room in reach:
        ...
        while True:
            ...
            door, _ = self.add_door(i, j, k, color, False, self_locking=self_locking)
