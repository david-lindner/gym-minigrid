...
class OneHotFullyObsWrapper(gym.core.ObservationWrapper):
    """
    Wrapper to get a one-hot encoding of a fully observable grid.

    TODO DL: this is not properly tested!
    """
    ...
    def observation(self, obs):
        ...
        for i in range(self.env.width):
            for j in range(self.env.height):
                ...
                if obj is None:
                    if list(self.env.agent_pos) == [i, j]:
                        type, color, state = (
                            OBJECT_TO_IDX["agent"],
                            self.env.agent_dir,
                            0,
